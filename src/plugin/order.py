...
from shared.db.models.order import Order
...
class OrderPlugin(BasePlugin):
    ...
    def before_request(self):
        LOGGER.info('Start scrapping orders')

    def request(self):
        with Database() as session:
            with Browser() as browser_session:
                url = self.url(Category.ORDER.value)
                loop = True
                while loop:
                    try:
                        ...
                        if len(elements) == 0:
                            LOGGER.info('Done, no more orders')
                            return
                        ...
                        for element in elements:
                            ...
                            if not created:
                                LOGGER.info(f'Order {model.url} ...skipped (duplicate)')
                                loop = False
                            else:
