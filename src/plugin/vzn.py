...
from shared.db.models.vzn import VZN


class VZNPlugin(BasePlugin):
    def __init__(self):
        super().__init__()

    def before_request(self):
        LOGGER.info('Start scrapping VZNs')

    def request(self):
        with Database() as session:
            with Browser() as browser_session:
                url = self.url(Category.VZN.value)
                loop = True
                while loop:
                    try:
                        ...
                        for element in elements:
                            ...
                            if is_pdf:
                                ...
                                if not created:
                                    LOGGER.info(f'VZN {model.url} ...skipped (duplicate)')
                                    loop = False
                                else:
                                    LOGGER.info(f'{model.url} ...added')
                            else:
                                LOGGER.warning(f'VZN {model.url} ...skipped (not PDF)')
    ...
    def after_request(self):
        LOGGER.info('Finished scrapping VZNs')
