...
from shared.db.models.table import Table
    ...
    def request(self):
        with Database() as session:
            with Browser() as browser_session:
                url = self.url(Category.TABLE.value)
                loop = True
                while loop:
                    try:
                        ...
                        if len(elements) == 0:
                            LOGGER.info('Done, no more table docs')
                            return
                        ...
                        for element in elements:
                            ...
                            if is_pdf:
                                ...
                                if not created:
                                    LOGGER.info(f'Table doc {model.url} ...skipped (duplicate)')
                                    loop = False
                                else:
                                    LOGGER.info(f'{model.url} ...added')
                            else:
                                LOGGER.warning(f'Table doc {model.url} ...skipped (not PDF)')
