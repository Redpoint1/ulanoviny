...
from shared.db.models.transcript import Transcript
...
class TranscriptPlugin(BasePlugin):
    ...
    def before_request(self):
        LOGGER.info('Start scrapping transcripts')

    def request(self):
        with Database() as session:
            with Browser() as browser_session:
                url = self.url(Category.TRANSCRIPT.value)
                loop = True
                while loop:
                    try:
                        ...
                        if len(elements) == 0:
                            LOGGER.info('Done, no more transcripts')
                            return
                        ...
                        for element in elements:
                            ...
                            if is_pdf:
                                ...
                                if not created:
                                    LOGGER.info(f'Transcript {model.url} ...skipped (duplicate)')
                                    loop = False
                                else:
                                    LOGGER.info(f'{model.url} ...added')
                            else:
                                LOGGER.warning(f'Transcript {model.url} ...skipped (not PDF)')
    ...
    def after_request(self):
        LOGGER.info('Finished scrapping transcripts')
